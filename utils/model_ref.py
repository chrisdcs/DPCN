...
class MM_Conv_Layer(nn.Module):
    ...
    def init_variables(self, batch_size):
        self.u = torch.abs(torch.rand(batch_size, self.B.shape[-1], 1).to(self.device))
        self.u = self.u / torch.norm(self.u, dim=1, keepdim=True)
        self.x = torch.rand(batch_size, self.C.shape[-1], 1).to(self.device)
    
    def reset(self):
        self.x, self.u = None, None
    
    def zero_grad_dict_C(self):
        self.C.grad[self.C==0] = 0

    def zero_grad_dict_B(self):
        self.B.grad[self.B==0] = 0
    ...
    def update_state(self, x_k, u_k, Cy):
        ...
        for _ in range(self.n_x):
            W_inv = torch.div(torch.abs(x_k), self.gamma0)# * gamma)
        ...
        x_index = x_k / torch.norm(x_k)
        x_index = torch.abs(x_index) < 1e-8
        x_k[x_index] = 0
    ...
    def update_cause(self, x_k, u_k, td):
        ...
        for _ in range(self.n_u):
            ...
            if td is None:
                ...
                W_inv = torch.div(torch.abs(u_k), self.beta+torch.abs(u_k))
                u_k = W_inv * (self.B.transpose(-1,-2) @ (self.gamma0 * x_k * torch.exp(-self.B @ u_k))+0.01 * td)
        
        u_index = u_k / torch.norm(u_k)
        u_index = torch.abs(u_index) < 1e-8
        u_k[u_index] = 0
        
        return u_k
    
    def forward(self, x_k, u_k, indices):
        recon = self.C @ x_k
        #recon_x = self.B @ u_k
        gamma = torch.exp(-self.B @ u_k).view(-1, self.u_ch, self.state_shape[-2], self.state_shape[-1])
        gamma = gamma[:, :, self.k_size//2:-(self.k_size//2), self.k_size//2:-(self.k_size//2)]
        gamma = self.unpool(gamma, indices)
        gamma = gamma.flatten(1).unsqueeze(-1)
        cause_state = gamma * torch.abs(x_k)
